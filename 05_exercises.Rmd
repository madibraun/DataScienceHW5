...
```{r}
anim_save("tomato_anim.gif")
```


```{r}
knitr::include_graphics("tomato_anim.gif")
```
...
mallorca <- get_stamenmap(
  bbox = c(left = 2.2378, bottom = 39.4512, right = 2.9863, top = 39.7686),
  maptype = "terrain",
  zoom = 10
)
...
```{r}
anim_save("heather_anim.gif")
```
  
```{r}
knitr::include_graphics("heather_anim.gif")
```
...
covid_anim <- ggplot(data = covid_new,
  ...
  geom_point(aes(group = state),
             color = "red")+
  geom_text(aes(label = state, group = state)) +
